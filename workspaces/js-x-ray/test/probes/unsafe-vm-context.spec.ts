...
`;
    ...
    test("should detect inlined runInContext call", () => {
      const code = `
import vm from "node:vm";
...
const result = (new vm.Script(code)).runInContext(createContex({}));
...
`;
    ...
    test("should detect re-assigned inlined runInContext", () => {
      const code = `
import vm from "node:vm";
...
const code = \`
 ...
 const runInContext = (new vm.Script(code)).runInContext;

runInContext(createContex({}));
`;
