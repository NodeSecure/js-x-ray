...
describe("isUnsafeCommand probe", () => {
  it("should detect csrutil command", () => {
  ...
  it("should detect hidden csrutil command", () => {
    COMMAND_TYPES.forEach((cmdType) => {
      const isArrayBased = cmdType === "spawn" || cmdType === "spawnSync";
      const args = isArrayBased ? `"csrutil", ["status"]` : `"csrutil status"`;
  ...
  it("should detect csrutil command with require", () => {
    COMMAND_TYPES.forEach((cmdType) => {
      const isArrayBased = cmdType === "spawn" || cmdType === "spawnSync";
  ...
  it("should transform template literal arg who are to template literal to literal", () => {
    const code = `
  ...
  it("should not detect non suspicious command", () => {
    COMMAND_TYPES.forEach((cmdType) => {
      const isArrayBased = cmdType === "spawn" || cmdType === "spawnSync";
  ...
  it("mydummyproject-zyp detection", () => {
  ...
  it("conservative mode: should not detect ls command", () => {
    const str = `
  ...
  it("aggressive mode: should detect all child_process usage (ls)", () => {
    const str = `
  ...
  it("aggressive mode: should detect echo command", () => {
    const str = `
  ...
  it("aggressive mode: should still detect curl command", () => {
    const str = `
      const { exec } = require("child_process");
  ...
  it("aggressive mode: should handle spawn with array arguments", () => {
    const str = `
      const { spawn } = require("child_process");
      spawn("node", ["--version"]);
    `;
